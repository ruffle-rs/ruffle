...
/// A [`FontRenderer`] that delegates glyph rasterization to a pluggable
/// JS bridge installed on `globalThis.__ruffleCustomFontRenderer` on
/// every font request by `WebUiBackend::load_device_font` and passed to
/// this struct's constructor; all subsequent calls are made via
/// reflection. See `web/packages/core/src/internal/custom-font-bridge.ts`
/// for the TS types and `web/src/ui.rs` for the plumbing.
///
