...
impl TestInfo {
    ...
    pub async fn start(&self) -> Result<ActiveTest, String> {
        ...
        if !self.0.should_run(false, true, &environment) {
...
impl RenderInterface for WgpuRenderInterface {
    ...
    fn capture(&self, backend: &mut (dyn RenderBackend + 'static)) -> RgbaImage {
        let renderer = <dyn Any>::downcast_mut::<WgpuRenderBackend<TextureTarget>>(backend)
            .unwrap();
