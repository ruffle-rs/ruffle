...
function cargoBuild({
    ...
    let args = ["build", "--locked", "--target", "wasm32-unknown-unknown", "-p", "web_test_runner"];
    if (!extensions) {
        args.push("-Z");
...
function buildWasm(
    ...
    if (wasmSource === "cargo" || wasmSource === "cargo_and_store") {
        ...
        originalWasmPath = `../../../target/wasm32-unknown-unknown/${profile}/web_test_runner.wasm`;
        if (wasmSource === "cargo_and_store") {
            copyFileSync(originalWasmPath, `../../build/${filename}.wasm`);
        }
    } else if (wasmSource === "existing") {
        originalWasmPath = `../../build/${filename}.wasm`;
    } else {
        throw new Error(
...
buildWasm("web-wasm-extensions", "web_test_runner", hasWasmOpt, true, wasmSource);
