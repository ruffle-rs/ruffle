 ...
 *
 * Ruffle consumes these per size: text layout (line heights, baselines)
 * and the text measurements reported to ActionScript are derived from
 * the metrics measured at the size actually being displayed, not from a
 * canonical size scaled down. Pixel-locked backends (GDI, swash) should
 * therefore report **whole pixels**, consistent with the glyph bitmaps
 * they rasterize — fractional values here propagate into fractional
 * layout positions in content that sizes itself around text (e.g. Flex
 * chrome), pushing vector graphics off the pixel grid.
 */
export interface FontMetrics {
    ...
    leading: number;
    /**
     * Typographic ascent (OS/2 `sTypoAscender`), in pixels. Optional.
     *
     * When present, the Flash Text Engine (Spark text) reports this instead
     * of {@link ascent} for `TextLine` measurements, matching Flash Player,
     * which sizes FTE text with the typographic metrics while classic text
     * fields keep the (taller) cell {@link ascent}/{@link descent}. Omit both
     * typo fields to have FTE fall back to the cell metrics. Report whole
     * pixels here too, for the same reason as {@link ascent}.
     */
    typoAscent?: number;
    /**
     * Typographic descent (OS/2 `-sTypoDescender`), in pixels. Optional; see
     * {@link typoAscent}.
     */
    typoDescent?: number;
}
