...
struct RegisteredFace {
    ...
    units_per_em: u16,
    /// OS/2 typographic ascender/descender in font units (`sTypoAscender`,
    /// `sTypoDescender`), cached at registration. Reported to the Flash Text
    /// Engine so Spark text is sized like Flash Player, while classic text
    /// fields keep the cell ascent/descent. `None` when the font has no OS/2.
    typo_metrics: Option<(i16, i16)>,
}
...
pub fn register_font_data(source: &str, bytes: Uint8Array) {
    ...
    for index in 0..face_count {
        ...
        // OS/2 typographic ascender/descender (font units) for the Flash Text
        // Engine; `None` when the font has no OS/2 table (FTE then falls back
        // to the cell metrics, like a renderer that reports no typo metrics).
        let typo_metrics = face
            .tables()
            .os2
            .map(|os2| (os2.typographic_ascender(), os2.typographic_descender()));
...
pub fn create_renderer(family: &str, bold: bool, italic: bool) -> Option<CustomFontRenderer> {
    let key = FaceKey {
        family_lower: family.to_lowercase(),
        bold,
        italic,
...
impl CustomFontRenderer {
    ...
    pub fn get_font_metrics(&mut self, size_px: f64) -> JsValue {
        ...
        let obj = Object::new();
        // Report whole pixels, the same rounding `ensure_glyph` uses to pad
        // the bitmaps: Ruffle consumes these as exact per-size metrics, so
        // a fractional value here would push every layout position derived
        // from them (line heights, the measurements Flex bases its chrome
        // on) off the pixel grid.
        set_number(&obj, "ascent", f64::from(m.ascent.round()));
        set_number(&obj, "descent", f64::from(m.descent.round()));
        set_number(&obj, "leading", f64::from(m.leading.round()));
        // Typographic ascent/descent (OS/2 `sTypo*`) for the Flash Text
        // Engine, scaled to this size and rounded to whole pixels like the
        // cell metrics above. Classic text fields keep the cell ascent/descent;
        // Spark (FTE) prefers these, matching Flash Player. Omitted when the
        // font has no OS/2 table, so FTE falls back to the cell metrics.
        if let Some((typo_asc, typo_desc)) = self.face.typo_metrics {
            let upm = self.face.units_per_em;
            if upm > 0 && typo_asc > 0 {
                let scale = f64::from(size_px) / f64::from(upm);
                let ascent = (f64::from(typo_asc) * scale).round();
                // `sTypoDescender` is negative (below the baseline).
                let descent = (-f64::from(typo_desc) * scale).round();
                if ascent > 0.0 && descent >= 0.0 {
                    set_number(&obj, "typoAscent", ascent);
                    set_number(&obj, "typoDescent", descent);
                }
            }
        }
        obj.into()
