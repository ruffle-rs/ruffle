...
use windows::Win32::Graphics::Gdi::{
    CLIP_DEFAULT_PRECIS, CreateCompatibleDC, CreateFontW, DEFAULT_CHARSET, DEFAULT_PITCH, DeleteDC,
    DeleteObject, FF_DONTCARE, FIXED, FW_BOLD, FW_NORMAL, GGO_BITMAP, GGO_METRICS, GLYPHMETRICS,
    GetCharWidth32W, GetFontData, GetGlyphOutlineW, GetKerningPairsW, GetTextMetricsW, HDC, HFONT,
    HGDIOBJ, KERNINGPAIR, MAT2, NONANTIALIASED_QUALITY, OUT_DEFAULT_PRECIS, SelectObject,
    TEXTMETRICW,
};
...
pub struct GdiFontMetrics {
    /// GDI cell ascent (`tmAscent`). Used for classic text fields (mx
    /// components), matching Flash Player's device-font TextField.
    pub ascent: i32,
    /// GDI cell descent (`tmDescent`).
    pub descent: i32,
    pub leading: i32,
    /// Typographic ascent (OS/2 `sTypoAscender`), scaled to size. The Flash
    /// Text Engine (Spark) reports this instead of the cell ascent, matching
    /// Flash Player's FTE, which sizes text with the typographic metrics.
    pub typo_ascent: i32,
    /// Typographic descent (OS/2 `-sTypoDescender`), scaled to size.
...
struct SizedFont {
    ...
    hfont: HFONT,
    prev_obj: HGDIOBJ,
    /// GDI cell ascent (`tmAscent`), used to rasterize glyph bitmaps so the
    /// full cell — including diacritics that rise above the typographic
    /// ascent — is never clipped.
    ascent: i32,
    /// GDI cell descent (`tmDescent`), the bitmap's below-baseline height.
    descent: i32,
    leading: i32,
    /// Typographic ascent from OS/2 (`sTypoAscender`), scaled to this size.
    /// Reported to Ruffle for layout so text boxes match Flash Player, which
    /// sizes with the typographic metrics rather than the taller GDI cell.
    typo_ascent: i32,
    /// Typographic descent from OS/2 (`-sTypoDescender`), scaled to size.
    typo_descent: i32,
...
impl GdiFontRenderer {
    ...
    pub fn get_font_metrics(&self, size_px: u32) -> Result<GdiFontMetrics> {
        ...
        // Cell metrics in `ascent`/`descent` for classic text fields (mx); the
        // typographic metrics are reported separately and consumed only by the
        // Flash Text Engine (Spark), matching how Flash Player sizes each.
        Ok(GdiFontMetrics {
            ascent: font.ascent,
            descent: font.descent,
            leading: font.leading,
            typo_ascent: font.typo_ascent,
            typo_descent: font.typo_descent,
        })
    }
...
fn create_sized_font(
    ...
    unsafe {
        ...
        // Prefer the font's OS/2 typographic ascent/descent for layout (what
        // Flash Player uses); fall back to the GDI cell metrics for fonts
        // without a usable OS/2 table.
        let (typo_ascent, typo_descent) =
            os2_typo_metrics(hdc, size_px).unwrap_or((tm.tmAscent, tm.tmDescent));
...
/// Read a big-endian `u16` at `offset` bytes into the named SFNT table of
/// the font currently selected into `hdc`. Returns `None` if the table is
/// missing or too short.
unsafe fn read_table_u16(hdc: HDC, tag: u32, offset: u32) -> Option<u16> {
    let mut buf = [0u8; 2];
    let got = GetFontData(hdc, tag, offset, Some(buf.as_mut_ptr() as *mut _), 2);
    if got != 2 {
        return None;
    }
...
/// Typographic ascent/descent from the OS/2 table, scaled to `size_px` and
/// rounded to whole device pixels.
///
/// Flash Player's FTE sizes text with these OS/2 typographic metrics, so
/// reporting them makes Ruffle's text boxes match. The GDI cell metrics
/// (`tmAscent`/`tmDescent`) instead include the internal leading and run
/// ~2-4px taller. `GetOutlineTextMetricsW` does not expose the OS/2
/// `sTypo*` fields, so read them straight from the font tables.
///
/// Returns `None` when the font lacks an `OS/2`/`head` table; callers then
/// fall back to the GDI cell metrics.
unsafe fn os2_typo_metrics(hdc: HDC, size_px: u32) -> Option<(i32, i32)> {
    // SFNT table tags, byte-reversed for `GetFontData`'s little-endian DWORD.
    const OS2: u32 = u32::from_le_bytes([b'O', b'S', b'/', b'2']);
    const HEAD: u32 = u32::from_le_bytes([b'h', b'e', b'a', b'd']);
    // Field offsets within each table.
    const HEAD_UNITS_PER_EM: u32 = 18;
    const OS2_STYPO_ASCENDER: u32 = 68;
    const OS2_STYPO_DESCENDER: u32 = 70;

    let units_per_em = read_table_u16(hdc, HEAD, HEAD_UNITS_PER_EM)?;
    if units_per_em == 0 {
        return None;
    }
    let s_typo_ascender = read_table_u16(hdc, OS2, OS2_STYPO_ASCENDER)? as i16;
    let s_typo_descender = read_table_u16(hdc, OS2, OS2_STYPO_DESCENDER)? as i16;

    let scale = f64::from(size_px) / f64::from(units_per_em);
    let ascent = (f64::from(s_typo_ascender) * scale).round() as i32;
    // `sTypoDescender` is negative (below the baseline); report a positive depth.
    let descent = (f64::from(-s_typo_descender) * scale).round() as i32;
