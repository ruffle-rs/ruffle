...
const ROWS: [(CharacterId, CharacterId, [f32; 4]); 3] = [
    (10, 11, [20.0, 20.0, 40.0, 40.0]),
    (20, 21, [20.0, 20.0, 40.0, 40.0]),
    (30, 31, [100.0, 100.0, 120.0, 120.0]),
];
...
fn main() {
    ...
    for (row, &(gridded, plain, [x0, y0, x1, y1])) in ROWS.iter().enumerate() {
        let on_sprite = row == 1;
        for (col, &id) in [gridded, plain].iter().enumerate() {
            ...
            if on_sprite {
                tags.push(Tag::DefineSprite(Sprite {
                    id,
                    num_frames: 1,
                    tags: vec![Tag::PlaceObject(place(shape_id, 1)), Tag::ShowFrame],
                }));
            }
            let target = if on_sprite { id } else { shape_id };
            if col == 0 {
            ...
            let mut p = place(target, (row * 2 + col + 1) as Depth);
    ...
    for (row, &(gridded, plain, records)) in BUTTON_ROWS.iter().enumerate() {
        for (col, &id) in [gridded, plain].iter().enumerate() {
            ...
            let mut p = place(id, (7 + row * 2 + col) as Depth);
            p.matrix = Some(Matrix {
                a: Fixed16::from_f32(3.0),
                tx: px(10.0 + col as f32 * 195.0),
                ty: px(220.0 + row as f32 * 70.0),
                ..Matrix::IDENTITY
            });
            tags.push(Tag::PlaceObject(p));
        }
    ...
    let header = Header {
        ...
        version: 10,
        stage_size: rect(0.0, 0.0, 400.0, 360.0),
