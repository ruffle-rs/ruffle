...
var colorTransformAsObj = {alphaMultiplier: 0.5, alphaOffset: 20, blueMultiplier: 0.5, blueOffset: 20, greenMultiplier: 0.5, greenOffset: 20, redMultiplier: 0.5, redOffset: 20};
var arrayZeroToTwoFiveSix = new Array(256);
var arrayTwoFiveSixToFiveTwelve= new Array(256);
var arrayOnlySpecificPixels = [];

for (var i = 0; i < arrayZeroToTwoFiveSix.length; i++) {
    arrayZeroToTwoFiveSix[i] = 256 - i;
}
for (var i = 0; i < arrayTwoFiveSixToFiveTwelve.length; i++) {
    arrayTwoFiveSixToFiveTwelve[i] = 256 + i;
}
arrayOnlySpecificPixels[0xABCABCAB] = 0x98765432;
arrayOnlySpecificPixels[0xFFCABCAB] = 0x87654321;
...
function main() {
    ...
    opaqueBmd.setPixel32(1, 1, 0xABCABCAB);
    transparentBmd = new BitmapData(3, 3, true, 0x12345678);
    transparentBmd.setPixel32(1, 1, 0xABCABCAB);

    // dumpBmdAfter, name, [args...]
    var functionsAndArgs = [
        [true, "paletteMap", opaqueBmd, rect, point, arrayZeroToTwoFiveSix, arrayTwoFiveSixToFiveTwelve, arrayZeroToTwoFiveSix, arrayTwoFiveSixToFiveTwelve]
    ];
...
function generateBadArguments(good) {
    ...
    if (good == colorTransform) {
        result.push(colorTransformAsObj);
    }
    if (good == arrayOnlySpecificPixels) {
        result.push(arrayZeroToTwoFiveSix);
        result.push(arrayTwoFiveSixToFiveTwelve);
    }
    if (good == arrayZeroToTwoFiveSix) {
        result.push(arrayOnlySpecificPixels);
        result.push(arrayTwoFiveSixToFiveTwelve);
    }
    if (good == arrayTwoFiveSixToFiveTwelve) {
        result.push(arrayZeroToTwoFiveSix);
        result.push(arrayOnlySpecificPixels);
    }
...
function callWithSpecificArgs(createObject, callback, functionName, args) {
    var argStr = "";
    ...
    var f = object[functionName].apply(object, args);
...
function valueToString(v) {
    ...
    if (v === colorTransformAsObj) {
        return "colorTransformAsObj";
    }
    if (v === arrayOnlySpecificPixels) {
        return "arrayOnlySpecificPixels";
    }
    if (v === arrayZeroToTwoFiveSix) {
        return "arrayZeroToTwoFiveSix";
    }
    if (v === arrayTwoFiveSixToFiveTwelve) {
        return "arrayTwoFiveSixToFiveTwelve";
    }
