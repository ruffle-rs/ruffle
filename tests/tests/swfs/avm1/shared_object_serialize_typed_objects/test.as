...
trace("--- Generating SharedObject (.sol) ---");

var so = SharedObject.getLocal("amf_object_payload");
so.clear();

so.data.objPlain = objPlain;
so.data.objUnregistered = objUnregistered;
so.data.objRegistered = objRegistered;
so.data.objMismatched = objMismatched;
so.data.objForgedConstructor = objForgedConstructor;
so.data.objForgedProto = objForgedProto;
so.data.objDoubleRegistered = objDoubleRegistered;
