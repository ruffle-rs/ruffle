package {
    ...
    public class Test extends MovieClip {
        ...
        private function contextCreated(event:Event):void {
            ...
            var redGreen = new BitmapData(size, size, true, 0xFFFFFFFF);
            redGreen.fillRect(new Rectangle(0, 0, halfSize, halfSize), 0xFFFF0000);
            redGreen.fillRect(new Rectangle(halfSize, 0, halfSize, halfSize), 0xFF00FF00);
            redGreen.fillRect(new Rectangle(0, halfSize, halfSize, halfSize), 0xFF0000FF);
            redGreen.fillRect(new Rectangle(halfSize, halfSize, halfSize, halfSize), 0xFFFF00FF);

            trace("Corner: " + redGreen.getPixel32(size - 1, size - 1).toString(16));

            var redGreenTexture = renderContext.createRectangleTexture(size, size, "bgra", false);
            ...
            var offsetVec = Vector.<Number>([-0.7, -0.4, 0, 0]);
            ...
            for each (var textureFilter in [Context3DTextureFilter.NEAREST, Context3DTextureFilter.LINEAR]) {
                // Rectangle textures only support CLAMP
                for each (var wrapMode in [Context3DWrapMode.CLAMP]) {
