...
fn image_input_as_texture<'a>(
    ...
    match input {
        ...
        ImageInputTexture::Floats {
            ...
            let (channels, padded) = match data {
                FloatPixelData::R(r) => (1, Cow::Borrowed(r.as_flattened())),
                FloatPixelData::Rg(rg) => (2, Cow::Borrowed(rg.as_flattened())),
                ...
                FloatPixelData::Rgba(rgba) => (4, Cow::Borrowed(rgba.as_flattened())),
            };

            let padded_bytes = bytemuck::cast_slice::<f32, u8>(padded.as_ref());
