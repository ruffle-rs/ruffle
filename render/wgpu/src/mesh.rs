    ...
    pub fn new(
        ...
        let colors = if gradient.records.is_empty() {
            ...
            for t in 0..GRADIENT_SIZE {
                ...
                let a = if t <= last_record.ratio as usize {
                    // We are before the first gradient record
                    0.0
                } else if t > next_record.ratio as usize
                    || (last_record.ratio == next_record.ratio && last != next)
                {
                    // We are after the last record, or the next record is identical to the last one,
                    // in which case we need to go to the current color.
                    1.0
                } else {
