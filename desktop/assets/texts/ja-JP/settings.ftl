network-settings = ネットワーク設定
player-settings = プレイヤー設定
movie-parameters = 映像パラメーター
custom-base-url = カスタムベースURL
spoof-swf-url = SWFのURLを偽装
proxy = プロキシ
upgrade-http = HTTPをHTTPSに置き換え
upgrade-http-check = 置き換える
open-url-mode = URLを開く際の処理
open-url-mode-allow = 許可する
open-url-mode-confirm = 尋ねる
open-url-mode-deny = 拒否する
load-behavior = 読み込み方法
load-behavior-streaming = ストーリーミング
load-behavior-delayed = 遅延
load-behavior-blocking = ブロック
max-execution-duration = 最大実行時間
max-execution-duration-suffix = { " " } 秒
quality = クオリティ
quality-low = 低
quality-medium = 中
quality-high = 高
quality-best = 最適
quality-high8x8 = 高(8x8)
quality-high8x8linear = 高(8x8) リニアsRGB
quality-high16x16 = 高(16x16)
quality-high16x16linear = 高(16x16) リニアsRGB
align = ステージ位置
align-center = 中心
align-left = 左
align-right = 右
align-top = 上
align-bottom = 下
align-top-left = 左上
align-bottom-left = 左下
align-top-right = 右上
align-bottom-right = 右下
align-force = 強制
scale-mode = スケールモード
scale-mode-exactfit = フィット
scale-mode-noborder = ふちなし
scale-mode-noscale = スケールなし
scale-mode-showall = 全て表示
scale-mode-force = 強制
warn-if-unsupported = サポートされていない場合は警告
warn-if-unsupported-check = 警告
player-version = プレイヤー バージョン
custom-framerate = カスタムフレームレート
custom-framerate-suffix = { " " } fps
dummy-external-interface = ダミー外部インターフェイス
dummy-external-interface-check = 有効
