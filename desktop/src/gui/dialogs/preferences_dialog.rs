...
impl PreferencesDialog {
    pub fn new(preferences: GlobalPreferences) -> Self {
        ...
        Self {
            ...
            flags_cli: preferences.cli.flags.clone(),
            flags_changed: false,
                ...
                .body(|mut body| {
                    for &flag in CompatibilityFlags::all_flags() {
                        ...
                        body.row(18.0, |mut row| {
                            row.col(|ui| {
                                if self.flags_cli.enabled(flag).is_ok() {
                            ...
                            row.col(|ui| {
                                if let Ok(mut cli_value) = self.flags_cli.enabled(flag) {
    ...
    fn save(&mut self) {
        if let Err(e) = self.preferences.write_preferences(|preferences| {
            ...
            if self.flags_changed {
                preferences.set_flags(self.flags.clone());
            }
        }) {
