...
impl<'gc> AvmStringInterner<'gc> {
        ...
        F: FnOnce(S) -> Gc<'gc, AvmStringRepr<'gc>>,
    {
        match self.interned.entry(mc, s.deref()) {
            WeakSetEntry::Occupied(strong) => AvmAtom(strong),
            WeakSetEntry::Vacant(vacant) => AvmAtom(vacant.insert(mc, f(s))),
        }
    }
    ...
    pub(super) fn get(&mut self, mc: &Mutation<'gc>, s: &WStr) -> Option<AvmAtom<'gc>> {
        match self.interned.entry(mc, s) {
            WeakSetEntry::Occupied(strong) => Some(AvmAtom(strong)),
            WeakSetEntry::Vacant(_) => None,
        }
    }
...
enum WeakSetEntry<'a, 'gc, T: 'gc> {
    Occupied(Gc<'gc, T>),
    Vacant(WeakSetVacant<'a, 'gc, T>),
}

struct WeakSetVacant<'a, 'gc, T: 'gc> {
    set: &'a mut WeakSet<'gc, T>,
    hash: u64,
}

impl<'a, 'gc, T: Hash + 'gc> WeakSetVacant<'a, 'gc, T> {
...
impl<'gc, T: Hash + 'gc> WeakSet<'gc, T> {
    ...
    fn entry<Q>(&mut self, mc: &Mutation<'gc>, key: &Q) -> WeakSetEntry<'_, 'gc, T>
    where
        T: Borrow<Q>,
        Q: Hash + Eq + ?Sized,
    {
            ...
            match table.find_entry(hash, |weak| {
                ...
                Ok(occupied) => {
                    if let Some(strong) = occupied.get().upgrade(mc) {
                        return WeakSetEntry::Occupied(strong);
                    } else {
                        occupied.remove();
                    }
                }
                Err(_) => return WeakSetEntry::Vacant(WeakSetVacant { set: self, hash }),
            }
        }
    }
