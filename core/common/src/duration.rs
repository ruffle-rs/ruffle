...
//!
//! This type is used instead of `std::time::Duration` because audio calculations
//! require ~16 significant figures of precision, which f64 provides but
//! `std::time::Duration` (nanosecond resolution) does not.
...
/// the precision needed for audio calculations.
