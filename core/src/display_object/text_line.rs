...
impl<'gc> TextLine<'gc> {
    ...
    ///
    /// This method assumes that `self` and `line_2` aren't the same line, and
    /// that both lines are in the same text block.
    pub fn find_lines_between(self, line_2: TextLine<'gc>) -> Vec<TextLine<'gc>> {
        ...
        for line in self.next_lines() {
            result.push(line);
            if DisplayObject::ptr_eq(line.into(), line_2.into()) {
                return result;
            }
        }
        ...
        unreachable!("Both lines should be in the same TextBlock!");
    }
}
