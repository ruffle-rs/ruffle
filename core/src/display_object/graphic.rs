...
impl<'gc> Graphic<'gc> {
    ...
    fn sliced_handle(
        ...
        let key = Scale9Key::new(scale9, to_grid_space).with_tessellation_scale(world_scale);
...
impl<'gc> TDisplayObject<'gc> for Graphic<'gc> {
    ...
    fn render_self(self, context: &mut RenderContext) {
        ...
        if let Some(drawing) = self.0.drawing.get() {
            ...
            if let Some(scale9) = self.active_scaling_grid() {
                drawing.render_scale9(context, &scale9, None);
            } else if let Some(scale9) = self.inherited_scaling_grid() {
                // The parent's grid is in the parent's space, so fold in the child's matrix.
                drawing.render_scale9(context, &scale9, Some(self.base().matrix()));
            } else {
            ...
            if let Some((scale9, to_grid_space)) = scale9 {
                ...
                if let Some(handle) =
                    self.sliced_handle(context, &scale9, to_grid_space, world_scale)
                {
                    let mut sliced = transform.clone();
                    sliced.matrix *= scale9.unscale(to_grid_space);
