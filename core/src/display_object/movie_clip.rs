...
impl<'gc> MovieClip<'gc> {
    ...
    pub fn new(movie: Gc<'gc, SwfMovie>, context: &mut UpdateContext<'gc>) -> Self {
        let mc = context.gc();
    ...
    pub fn new_import_assets(
        ...
        context.library.register_movie_library(movie, lib, mc);
        let shared = Gc::new(
            mc,
    ...
    pub fn replace_with_movie(
        ...
        context
            .library
            .register_movie_library(movie, lib, context.gc());
        ...
        self.set_library(context.gc(), lib);
    }
    ...
    pub fn init_clip_event_handlers(
        ...
        let mc = context.gc();
        unlock!(Gc::write(mc, self.0), MovieClipData, clip_event_handlers)
    ...
    fn instantiate_child(
        ...
        let movie = self.movie();
        let library = self.movie_library().unwrap();
        match library
            .borrow()
            .instantiate_by_id(id, context.gc_context, movie, library)
        {
            Some(child) => {
...
impl<'gc, 'a> MovieClipShared<'gc> {
    ...
    fn define_shape(
        ...
        let id = swf_shape.id;
        let lib = self.library().unwrap();
        let graphic = Graphic::from_swf_tag(context, swf_shape, self.movie(), lib);
        lib.borrow_mut(context.gc())
    ...
    fn define_edit_text(
        ...
        let edit_text = EditText::from_swf_tag(context, self.movie(), swf_edit_text, lib);
        lib.unwrap()
            .borrow_mut(context.gc())
        ...
        if !eager_tags.symbolclass_names.is_empty() {
            ...
            for (class_name, id) in eager_tags.symbolclass_names {
                ...
                match Avm2::lookup_class_for_character(&mut activation, self, domain, name, id) {
                    Ok(class_object) => {
                        let movie = library_gc.borrow().movie();
                        ...
                        activation
                            ...
                            .set_class_symbol(class_def, movie, id, mc);
...
impl<'gc> MovieClipShared<'gc> {
    fn empty(movie: Gc<'gc, SwfMovie>) -> Self {
...
impl<'gc> ClipEventHandler<'gc> {
    ...
    pub fn from_action_and_movie(
        ...
        let declared_events = other.events;
        ...
        Self {
            effective_events,
            key_code,
            action_data: SwfSlice::from(movie).to_unbounded_subslice(other.action_data),
        }
    }
}
