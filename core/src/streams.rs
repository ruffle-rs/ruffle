...
impl<'gc> NetStream<'gc> {
    ...
    /// Parse the F4V (MP4) container metadata and select the tracks to play,
    /// if not done already and enough of the stream has been buffered.
    ///
    /// `slice` must reference the full stream buffer. The current stream type
    /// must be `NetStreamType::F4v`.
    ///
    /// Returns `true` if the parsed context is available.
    fn f4v_parse_context(self, slice: &Slice) -> bool {
        let source = self.source();
        let buffer = slice.data();
        ...
        media_context.is_some()
    }

    /// Decode the F4V video track's samples up to `max_time` (in ms).
    ///
    /// `slice` must reference the full stream buffer. The current stream type
    ...
    fn f4v_advance_video(
        ...
        // `video_stream` and `next_frame` are written back at the end.
        let (mp4, video_track_id, mut video_stream, mut next_frame) =
            match &*source.stream_type.borrow() {
                Some(NetStreamType::F4v {
                    ...
                    next_frame,
                    ..
                }) => (mp4.clone(), *video_track_id, *video_stream, *next_frame),
                _ => unreachable!(),
            };
        ...
        let trk = mp4.tracks().get(&vti).unwrap();
        ...
        if video_stream.is_none() {
            match context.video.register_video_stream(
                ...
                Ok(new_handle) => {
                    ...
                    let ccfg = trk.raw_codec_config(&mp4).unwrap();
                    if let Err(e) = context
                        .video
        ...
        let exhausted = if let Some(video_handle) = video_stream {
            ...
            if let Some(first_smpl) = trk.samples.get(next_frame as usize) {
                let first_time_ms =
                    first_smpl.decode_timestamp as f64 * 1000.0 / first_smpl.timescale as f64;
    ...
    pub fn tick(self, context: &mut UpdateContext<'gc>, dt: FloatDuration) {
        ...
        if matches!(
            ...
            if !self.f4v_parse_context(&slice) {
                return;
            }
