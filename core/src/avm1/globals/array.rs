...
pub fn splice<'gc>(
    ...
    let result_array: Object<'gc> = ArrayObject::empty(
        activation.context.gc_context,
        Some(activation.context.avm1.prototypes.array),
    )
    .into();
    ...
    let (from, to) = if items.len() as i32 > delete_count {
        (length - 1, start + delete_count)
    } else {
        (start + delete_count, length - 1)
    };
    for i in from..=to {
        if this.has_element(activation, i) {
            let element = this.get_element(i);
...
pub fn create_proto<'gc>(
    ...
    let array = ArrayObject::empty(gc_context, Some(proto));
    let object = array.as_script_object().unwrap();
