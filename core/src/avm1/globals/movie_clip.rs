...
fn attach_movie<'gc>(
    ...
    if let Some(new_clip) = activation
        .context
        .library
        .library_for_movie_gc(movie_clip.movie(), activation.gc())
        .and_then(|l| {
            l.borrow()
                .instantiate_by_export_name(export_name, activation.gc(), movie_clip.movie())
        })
    {
...
fn get_bounds<'gc>(
    ...
    if let Some(target) = target {
        if !activation.context.avm1.get_use_new_invalid_bounds_value() {
            ...
            if activation.swf_version() >= 8 || activation.context.root_movie().version() >= 8 {
