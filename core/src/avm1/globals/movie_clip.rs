...
fn attach_movie<'gc>(
        ...
        .and_then(|l| l.instantiate_by_export_name(export_name, activation.gc()))
    {
        ...
        Ok(Value::or_undef(new_clip.object1()))
    } else {
...
fn create_text_field<'gc>(
    ...
    if activation.swf_version() >= 8 {
        ...
        Ok(Value::or_undef(text_field.object1()))
    } else {
        Ok(Value::Undefined)
    }
}
...
fn duplicate_movie_clip<'gc>(
    ...
    Ok(Value::or_undef(new_clip.and_then(|c| c.object1())))
}
    ...
    if activation.swf_version() >= 7 {
        ...
        match movie_clip.child_by_depth(depth) {
            Some(child) => {
                ...
                Ok(Value::or_undef(
                    child.object1().or_else(|| movie_clip.object1()),
                ))
            }
...
fn load_movie<'gc>(
    ...
    let target_obj = Value::or_undef(target.object1()).coerce_to_object(activation);
...
fn load_variables<'gc>(
    ...
    let method = args.get(1).cloned().unwrap_or(Value::Undefined);
    let method = NavigationMethod::from_method_str(&method.coerce_to_string(activation)?);
    let target = Value::or_undef(target.object1()).coerce_to_object(activation);
...
fn transform<'gc>(
    ...
    let this = Value::or_undef(this.object1());
    let cloned = constructor.construct(activation, &[this])?;
    Ok(cloned)
}
