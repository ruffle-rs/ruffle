...
impl<'gc> Avm1Function<'gc> {
    ...
    fn load_this(&self, frame: &mut Activation<'_, '_, 'gc>, this: Value<'gc>, preload_r: &mut u8) {
    ...
    fn load_root(&self, frame: &mut Activation<'_, '_, 'gc>, preload_r: &mut u8) {
    ...
    fn load_parent(&self, frame: &mut Activation<'_, '_, 'gc>, preload_r: &mut u8) {
    ...
    fn load_global(&self, frame: &mut Activation<'_, '_, 'gc>, preload_r: &mut u8) {
...
macro_rules! constructor_to_fn {
    ($f:expr) => {{
        fn _constructor_fn<'gc>(
            activation: &mut $crate::avm1::activation::Activation<'_, '_, 'gc>,
