...
impl<'a, 'gc> Activation<'a, 'gc> {
    ...
    pub fn run_child_frame_for_action<S: Into<Cow<'static, str>>>(
        ...
        let clip_obj =
            Value::or_undef(active_clip.object1()).coerce_to_object(&mut parent_activation);
    ...
    fn action_call_function(&mut self) -> Result<FrameControl<'gc>, Error<'gc>> {
        ...
        let result = variable.call_with_default_this(
            Value::or_undef(self.target_clip_or_root().object1()).coerce_to_object(self),
            fn_name,
            self,
            &args,
        )?;
        ...
        if action.is_load_vars() {
            ...
            if is_load_vars {
                if let Some(clip_target) = clip_target {
                    let target_obj = Value::or_undef(clip_target.object1()).coerce_to_object(self);
    ...
    fn action_set_target_2(&mut self) -> Result<FrameControl<'gc>, Error<'gc>> {
        ...
        let clip_obj =
            Value::or_undef(self.target_clip_or_base_clip().object1()).coerce_to_object(self);
    ...
    pub fn resolve_target_display_object(
        ...
        let start = Value::or_undef(start.object1()).coerce_to_object(self);
        Ok(self
    ...
    pub fn resolve_target_path(
        ...
        let (mut object, mut is_slash_path) = if path.starts_with(b'/') {
            path = &path[1..];
            (Value::or_undef(root.object1()).coerce_to_object(self), true)
        } else {
            (start, false)
        };
    ...
    pub fn resolve_variable_path<'s>(
        ...
        if let Some(object) = start.object1() {
            Ok(Some((object, path)))
        } else {
            Ok(None)
        }
    ...
    pub fn root_object(&self) -> Value<'gc> {
        Value::or_undef(self.base_clip().avm1_root().object1())
    }
    ...
    pub fn set_scope_to_display_object(&mut self, object: DisplayObject<'gc>) {
        self.scope = Gc::new(
            ...
            Scope::new(
                self.scope,
                ScopeClass::Target,
                Value::or_undef(object.object1()).coerce_to_object(self),
            ),
        );
    }
    ...
    fn set_target(&mut self, target: &WStr) -> Result<FrameControl<'gc>, Error<'gc>> {
        ...
        let start = Value::or_undef(base_clip.object1()).coerce_to_object(self);
        if target.is_empty() {
        ...
        let clip_obj = Value::or_undef(self.target_clip_or_root().object1()).coerce_to_object(self);
        self.set_scope(Scope::new_target_scope(self.scope(), clip_obj, self.gc()));
        Ok(FrameControl::Continue)
    }
}
