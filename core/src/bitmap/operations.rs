...
mod tests {
    ...
    /// Build a 3×3 BitmapData:
    ///   all pixels transparent (alpha = 0) except the centre (1,1) which is fully opaque (alpha = 0xFF).
    ///
    ///   0 1 2
    /// 0 . . .
    /// 1 . X .
    /// 2 . . .
    fn make_3x3_bmd<'gc>(mc: &Mutation<'gc>) -> BitmapData<'gc> {
        let pixels = vec![
            Color::rgba(0, 0, 0, 0),
            Color::rgba(0, 0, 0, 0),
            Color::rgba(0, 0, 0, 0),
        ...
        BitmapData::new_with_pixels(mc, 3, 3, true, pixels)
    }

    // --- hit_test_point ----------------------------------------------------------

    /// threshold=0 on an in-bounds transparent pixel returns false.
    /// threshold=0 is clamped to 1, so alpha(0) >= 1 is false.
    #[test]
    fn hit_test_point_threshold_zero_transparent_pixel_is_false() {
        let mut renderer = make_renderer();
        rootless_mutate(|mc| {
            let bmd = make_3x3_bmd(mc);
            // (0,0) has alpha = 0; threshold=0 clamped to 1 → alpha(0) >= 1 → false
            assert!(!hit_test_point(&mut renderer, bmd, 0, (0, 0)));
        });
    }

    /// threshold=0 on a fully-opaque pixel must still return true.
    #[test]
    fn hit_test_point_threshold_zero_opaque_pixel_is_true() {
        let mut renderer = make_renderer();
        rootless_mutate(|mc| {
            let bmd = make_3x3_bmd(mc);
            // (1,1) has alpha = 0xFF
            assert!(hit_test_point(&mut renderer, bmd, 0, (1, 1)));
        });
    }

    /// Out-of-bounds point always returns false regardless of threshold.
    #[test]
    fn hit_test_point_out_of_bounds_is_false() {
        let mut renderer = make_renderer();
        rootless_mutate(|mc| {
            let bmd = make_3x3_bmd(mc);
            assert!(!hit_test_point(&mut renderer, bmd, 0, (5, 5)));
        });
    }

    // --- hit_test_rectangle ------------------------------------------------------

    /// threshold=0, rectangle covering only transparent pixels → true.
    /// Flash Player treats threshold=0 as "alpha >= 0", so all in-bounds pixels match.
    #[test]
    fn hit_test_rectangle_threshold_zero_all_transparent_is_true() {
        let mut renderer = make_renderer();
        rootless_mutate(|mc| {
            let bmd = make_3x3_bmd(mc);
            // 1×1 region at (0,0): transparent pixel, alpha = 0; threshold=0 → true
            assert!(hit_test_rectangle(&mut renderer, bmd, 0, (0, 0), (1, 1)));
        });
    }

    /// threshold=0, rectangle covering the opaque centre pixel → true.
    #[test]
    fn hit_test_rectangle_threshold_zero_contains_opaque_is_true() {
        let mut renderer = make_renderer();
        rootless_mutate(|mc| {
            let bmd = make_3x3_bmd(mc);
            // 1×1 region at (1,1): centre pixel, alpha = 0xFF
            assert!(hit_test_rectangle(&mut renderer, bmd, 0, (1, 1), (1, 1)));
        });
    }

    // --- hit_test_bitmapdata -----------------------------------------------------

    /// threshold=0 on both bitmaps, overlapping region contains only transparent
    /// pixels → true. With threshold=0, `alpha >= 0` is always true, so every
    /// pixel counts as opaque. Verified against Adobe AIR 30.0 / adl.exe.
    #[test]
    fn hit_test_bitmapdata_threshold_zero_transparent_only_is_true() {
        let mut renderer = make_renderer();
        rootless_mutate(|mc| {
            // 1×1 fully-transparent bitmap
            let bmd_a = BitmapData::new_with_pixels(mc, 1, 1, true, vec![Color::rgba(0, 0, 0, 0)]);
            let bmd_b = BitmapData::new_with_pixels(mc, 1, 1, true, vec![Color::rgba(0, 0, 0, 0)]);
            // Both aligned at origin, both threshold=0
            assert!(hit_test_bitmapdata(
                &mut renderer,
                bmd_a,
                (0, 0),
                0,
    ...
    /// threshold=0 on both bitmaps, overlapping region contains an opaque pixel
    /// in both → true.
    #[test]
    fn hit_test_bitmapdata_threshold_zero_opaque_overlap_is_true() {
        let mut renderer = make_renderer();
        rootless_mutate(|mc| {
            let bmd_a =
                BitmapData::new_with_pixels(mc, 1, 1, true, vec![Color::rgba(255, 255, 255, 255)]);
            let bmd_b =
                BitmapData::new_with_pixels(mc, 1, 1, true, vec![Color::rgba(255, 255, 255, 255)]);
            assert!(hit_test_bitmapdata(
                &mut renderer,
