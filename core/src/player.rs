...
impl Player {
    ...
    fn update_focus_on_mouse_press<'gc>(
        ...
        if pressed_object.is_some_and(|int| int.is_focusable_by_mouse(context)) {
            tracker.set_by_mouse(pressed_object, context);
        } else if context.swf.is_action_script_3() || tracker // TODO ?
            .get()
            .is_some_and(|int| int.is_focusable_by_mouse(context))
        {
            // Need to clear the focus if an object focusable by mouse was focused.
