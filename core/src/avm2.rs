...
use crate::avm2::stack::{AvmStack, AvmStackFrame};
...
pub struct Avm2<'gc> {
    ...
    /// A dummy stack frame. This is created once and passed to whatever needs it.
    dummy_frame: AvmStackFrame<'gc, 'gc>,
...
impl<'gc> Avm2<'gc> {
    ...
    pub fn new(
        ...
        Self {
            player_version,
            player_runtime,
            stack: AvmStack::new(mc),
            dummy_frame: AvmStackFrame::for_data(Box::leak(Box::new([]))),
            scope_stack: Vec::new(),
    ...
    pub fn run_script_initializer(
        ...
        let stack_frame = stack.get_stack_frame(mc, method);
    ...
    pub fn dummy_stack_frame<'a>(&self) -> AvmStackFrame<'a, 'gc> {
        self.dummy_frame.clone()
    }
