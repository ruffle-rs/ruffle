...
pub struct MovieHandle {
    root: DynamicRoot<Rootable![SwfMovieGc<'_>]>,
    ptr: *const crate::tag_utils::SwfMovie,
}

impl MovieHandle {
    pub fn new<'gc>(context: &mut UpdateContext<'gc>, movie: SwfMovieGc<'gc>) -> Self {
        Self {
            root: context
                .dynamic_root
    ...
    pub fn fetch<'gc>(&self, dynamic_root_set: DynamicRootSet<'gc>) -> SwfMovieGc<'gc> {
        *dynamic_root_set.fetch(&self.root)
    }
}
