...
impl GlyphShape {
    ...
    /// Release GPU handles held by this glyph's bitmap, if any.
...
impl<'a> GlyphBitmap<'a> {
    ...
    pub fn get_handle_or_register(&self, renderer: &mut dyn RenderBackend) -> Option<BitmapHandle> {
        ...
        // First registration: consume the bitmap data
        if let Some(bitmap) = self.bitmap.take() {
            ...
            // Handle was cleared after registration — font is being discarded
            None
        }
    }
