...
impl<'gc> SoundObject<'gc> {
    ...
    pub fn play(self, queued: QueuedPlay<'gc>, activation: &mut Activation<'_, 'gc>) -> bool {
        ...
        match &mut *sound_data {
            SoundData::Empty | SoundData::Generated => {
                unreachable!("Generated sounds are started by Sound.play directly")
            }
    ...
    /// Returns `true` if this sound is in the `Generated` state.
    pub fn is_generated(self) -> bool {
        matches!(&*self.0.sound_data.borrow(), SoundData::Generated)
    }

    /// Transitions from `Empty` → `Generated`. Called when `Sound.play()`
    /// is invoked on a sound that has never been loaded.
    pub fn set_generated(self, mc: &Mutation<'gc>) {
        let mut sound_data =
            unlock!(Gc::write(mc, self.0), SoundObjectData, sound_data).borrow_mut();
        match &*sound_data {
            SoundData::Empty | SoundData::Generated => *sound_data = SoundData::Generated,
            SoundData::Loading { .. } => panic!("Tried to generate sound that is already Loading"),
            SoundData::Loaded { .. } => panic!("Tried to generate sound that is already Loaded"),
        }
        drop(sound_data);
        self.set_loading_state(SoundLoadingState::Generated);
    }
