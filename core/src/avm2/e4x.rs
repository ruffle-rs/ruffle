...
pub fn namespace_for_multiname<'gc>(
    ...
    } else if let Some(default_ns) = activation.default_xml_namespace() {
        Some(E4XNamespace::new_uri(
            default_ns.as_uri(&mut activation.context.strings),
        ))
    } else {
        None
    }
        ...
        let namespace = match ns {
            ...
            ResolveResult::Unbound => {
                ...
                if let Some(local_ns) = namespaces.iter().find(|ns| ns.prefix == Some(istr!(""))) {
                    ...
                    Some(E4XNamespace {
                        prefix: None,
                        uri: default_ns.as_uri(&mut activation.context.strings),
                    })
                } else {
                    None
                }
