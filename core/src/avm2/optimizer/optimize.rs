...
impl<'a, 'gc> AbstractStateRef<'a, 'gc> {
...
fn process_jump<'gc>(
    ...
    if worklist.iter().position(|x| *x == target_block_id).is_none() {
    ...
    for (i, op) in ops.iter().enumerate() {
        if op.get().can_throw_error() {
            ...
            for exception in method_exceptions {
                if current_idx >= exception.from_offset as usize
                    && current_idx < exception.to_offset as usize
                {
